...
export class QuestionnaireService {
  ...
  suitableHotel$: Observable<AnswerModel[]> = Observable.of(null);
  ...
  constructor(
    ...
    public setupNewQuestionnaire() {
      ...
      this.initialDomains = [];
        this.queryDomains();
        this.suitableHotel$ = Observable.of(null);
    }
    ...
    public queryHotels(): void {
        let myParams = new URLSearchParams()
        myParams.append('questionList',JSON.stringify(this.QUESTIONNAIRE.completedQuestionList))
        this.http.get(EndpointSettings.getSuitableHotelsEndpoint(), { search: myParams })
            .map(response => response.json()).subscribe(
            data => {

                console.log("Results from querying suitable hotels: " +JSON.stringify(data));
                this.suitableHotel$ = Observable.of(data);
